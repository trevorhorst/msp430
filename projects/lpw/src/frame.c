...
void canvas_draw_sprite(Canvas *canvas, const uint8_t* sprite, int sprite_w, int sprite_h,
                         ...
                         CanvasColor color)
{
    // Calculate the width byte-stride of the source sprite
    int sprite_bytes_per_row = (sprite_w + 7) / 8;

    for (int s_y = 0; s_y < sprite_h; s_y++) {
        int screen_y = y + s_y;

        for (int s_x = 0; s_x < sprite_w; s_x++) {
            int screen_x = x + s_x;

            // 1. Extract the pixel bit from the source sprite (MSB-left)
            int sprite_byte_idx = (s_y * sprite_bytes_per_row) + (s_x / 8);
            int sprite_bit_idx  = 7 - (s_x % 8);
            uint8_t pixel = (sprite[sprite_byte_idx] >> sprite_bit_idx) & 0x01;

            // 2. Write the pixel bit to the destination buffer if it's active
            if(pixel) {
                canvas_draw_point(canvas, screen_x, screen_y, WHITE, PIXEL_1X1);
            } else {
                canvas_draw_point(canvas, screen_x, screen_y, BLACK, PIXEL_1X1);
            }
        }
    }
}
