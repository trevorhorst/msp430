...
{
    i2c_write(device->bus, device->address, buffer, buffer_length);
}

void sht3xdis_read(sht3xdis_i2c_device *device, uint8_t *buffer, uint32_t buffer_length)
{
    i2c_read(device->bus, device->address, buffer, buffer_length);
}
...
sht3xdis_measurement sht3xdis_singleshot_measurement(sht3xdis_i2c_device *device,
    enum sht3xdis_repeatability repeat, uint8_t clock_stretch)
{
